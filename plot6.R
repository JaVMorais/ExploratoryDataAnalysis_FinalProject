...
PM25_data <- readRDS("summarySCC_PM25.rds") %>%
    ...
    ungroup %>%
    mutate(
        fips = factor(fips_names[
            match(fips, as.character(fips_names$code)),
            "names"
            ])
    ) %>%
    mutate(
...
(g + geom_bar(stat="identity", position=position_dodge()) +
        ...
        labs(x="Year",y="", title = "PM2.5 emissions by motor vehicles", subtitle = "Baltimore City and Los Angeles County") +
        theme_bw(base_size = 11) +
        theme(
            strip.background = element_blank(),
            strip.text.y = element_text(size = 8)
        )) %>%
    ggsave(filename = "plot6.png", device = "png", width=5, height = 7)